...
const TrackDetailsPage = ({ params }: { params: { song_id: string } }) => {
  ...
  const [activeTab, setActiveTab] = useState<"reviews" | "annotations">("reviews");

  // Mock data for track-specific stats
  const [trackStats] = useState({
    likes: 1247,
    reviews: 356,
    annotations: 89,
    avgRating: 4.3,
    ratingDistribution: [5, 15, 20, 40, 20] // percentages for 1-5 stars
  });
  ...
  const reviews: Review[] = [
    ...
    },
    {
      ...
      user: "Mike Rodriguez",
      rating: 4.5,
      content: "The production quality is insane. Best track of the year!",
      timestamp: "1 day ago",
      likes: 31,
    },
    {
      ...
      user: "Emma Davis",
      rating: 4.0,
      content: "Solid track with a beautiful melody. Worth multiple listens.",
  ...
  return (
    <div className="min-h-screen bg-[#FFFFF0]">
      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        ...
        <div className="flex flex-col md:flex-row gap-6 mb-8">
          ...
          <div className="w-full md:w-2/3">
            ...
            <div className="grid grid-cols-2 gap-3 mb-6">
              ...
              <div>
                <p className="text-xs text-[#A0A0A0]">Release Date</p>
                <p className="text-sm font-medium">
                  {new Date(track.album.release_date).toLocaleDateString()}
                </p>
              </div>
              <div>
                <p className="text-xs text-[#A0A0A0]">Duration</p>
        ...
        <div className="mb-8">
          <div className="grid grid-cols-4 gap-2 border-y border-[#D9D9D9] py-5">
            <div className="text-center">
              <div className="text-xl font-bold text-[#0C3B2E]">{trackStats.likes}</div>
              <div className="text-xs text-[#A0A0A0] flex items-center justify-center">
                <Heart className="w-3 h-3 mr-1" /> Likes
              </div>
            </div>
            <div className="text-center">
              <div className="text-xl font-bold text-[#0C3B2E]">{trackStats.reviews}</div>
              <div className="text-xs text-[#A0A0A0] flex items-center justify-center">
                <Star className="w-3 h-3 mr-1" /> Reviews
              </div>
            </div>
            <div className="text-center">
              <div className="text-xl font-bold text-[#0C3B2E]">{trackStats.annotations}</div>
              <div className="text-xs text-[#A0A0A0] flex items-center justify-center">
                <MessageCircle className="w-3 h-3 mr-1" /> Annotations
              </div>
            </div>
            <div className="text-center">
              <div className="text-xl font-bold text-[#0C3B2E]">{trackStats.avgRating}</div>
              <div className="text-xs text-[#A0A0A0]">Avg Rating</div>
            </div>
          </div>
        </div>
        ...
        <div className="mb-8">
          <h2 className="text-lg font-bold text-[#0C3B2E] mb-3">Rating Distribution</h2>
                ...
                <div className="flex-1 ml-2">
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <div 
                      ...
                      style={{ width: `${trackStats.ratingDistribution[index]}%` }}
                    ></div>
                  </div>
                </div>
        ...
        <div className="mb-6">
          ...
          {activeTab === 'reviews' && (
            <div className="mt-4 space-y-4">
              {reviews.map((review) => (
                <div key={review.id} className="bg-white border border-[#D9D9D9] rounded p-4">
          ...
          {activeTab === 'annotations' && (
            <div className="mt-4 space-y-4">
              {annotations.map((annotation) => (
                <div key={annotation.id} className="bg-white border border-[#D9D9D9] rounded p-4">
                  <div className="flex justify-between items-start mb-2">
